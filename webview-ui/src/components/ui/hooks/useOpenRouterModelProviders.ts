...
// Static catalog kept commented as a reference for the entry shape; the live
// catalog is fetched dynamically from the MatterAI backend. Entries spread an
// OSS_MODEL_BASE constant removed as dead code; restore from git history if
// re-enabling static entries.
...
async function getOpenRouterProvidersForModel(modelId: string, _baseUrl?: string, _apiKey?: string) {
	...
	// Dynamic fallback provider if model not in static map
	models["KiloCode"] = {
		maxTokens: 64000,
