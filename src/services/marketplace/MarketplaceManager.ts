...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".orbitalmodes")
			try {
			...
			// Check MCPs in .orbital/mcp.json
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".orbital", "mcp.json")
			try {
