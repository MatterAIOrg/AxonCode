...
function getEditingInstructions(diffStrategy?: DiffStrategy): string {
	...
	instructions.push(
		`- For editing files, you have access to these tools: file_edit (for targeted text replacements), file_write (for creating new files or complete file rewrites).`,
	)

	instructions.push(
		"- The file_edit tool performs targeted text replacements within a single file. Provide the current text (old_string) and the desired text (new_string). The tool shows a diff before applying changes so you can confirm the result.",
	)

	instructions.push(
		"- When using the file_write tool to create or overwrite a file, ALWAYS provide the COMPLETE file content in your response. This is NON-NEGOTIABLE. Partial updates or placeholders like '// rest of code unchanged' are STRICTLY FORBIDDEN. You MUST include ALL parts of the file, even if they haven't been modified.",
	)
...
${codebaseSearchRule}- When using the search_files tool${isCodebaseSearchAvailable ? " (after codebase_search)" : ""}, craft your regex patterns carefully to balance specificity and flexibility. Based on the user's task you may use it to find code patterns, TODO comments, function definitions, or any text-based information across the project. The results include context, so analyze the surrounding code to better understand the matches. Leverage the search_files tool in combination with other tools for more comprehensive analysis. For example, use it to find specific code patterns, then use read_file to examine the full context of interesting matches before using file_edit or file_write to make informed changes.
- When creating a new project (such as an app, website, or any software project), organize all new files within a dedicated project directory unless the user specifies otherwise. Use appropriate file paths when writing files, as the file_write tool will automatically create any necessary directories. Structure the project logically, adhering to best practices for the specific type of project being created. Unless otherwise specified, new projects should be easily run without additional setup, for example most projects can be built in HTML, CSS, and JavaScript - which you can open in a browser.
