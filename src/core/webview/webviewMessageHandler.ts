...
const skipPatterns = [
	...
	// Build directories - ALL FIXED with proper anchoring
	/\/dist\//,
	/\/build\//,
	/\/\.next\//,
	/\/node_modules\//,
	...
	// Generated files across frameworks - FIXED
	/\/generated\//,
	/\/\.generated\//,
	/\/\.gen\//,
	/\/auto-generated\//,
	...
	// Config and metadata files - FIXED
	/\.DS_Store$/,
	/Thumbs\.db$/,
	/desktop\.ini$/,
	/\/\.idea\//,
	...
	// Filter all files with .cursor in their path - FIXED
	/\/\.cursor\//,
	/\/\.github\//,
...
/**
 * Check if a file should be skipped from code review based on skip patterns
...
async function getGitChanges(cwd: string): Promise<{ files: any[]; gitDiff: string }> {
	try {
		...
		try {
			...
			// Filter out diffs for skipped files
			...
			for (const line of lines) {
				// Check for diff file header (e.g., "diff --git a/file.ts b/file.ts")
				...
				// Only include lines for files that are not skipped
				if (includeCurrentFile) {
					filteredDiffLines.push(line)
				}
			}

			gitDiff = filteredDiffLines.join("\n")
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "viewPendingFileDiffs" as any: {
			...
			// Use VS Code's native changes view to show all files in one view
			try {
				const { DIFF_VIEW_URI_SCHEME } = await import("../../integrations/editor/DiffViewProvider")

				const changes = Array.from(pendingEdits.entries()).map(([readablePath, edit]) => {
					// Get the new content from the last edit (most recent change)
					const lastEdit = edit.edits[edit.edits.length - 1]
					const newContent = lastEdit ? lastEdit.newContent : edit.originalContent

					return [
						vscode.Uri.file(edit.absolutePath),
						vscode.Uri.parse(`${DIFF_VIEW_URI_SCHEME}:${readablePath}`).with({
...
`
		...
		case "fileEditReviewAcceptAll": {
			...
			await vscode.commands.executeCommand("axon-code.fileEdit.acceptAll", currentTask.taskId)
			break
		}
		...
		case "fetchProfileDataRequest":
			try {
				...
				const url = "https://api.matterai.so/axoncode/profile"
