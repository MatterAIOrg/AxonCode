...
build_extension() {
  echo "🔨 Building the Orbital extension..."
...
if ! code --list-extensions 2>/dev/null | grep -q "matterai.axon-code"; then
  code --install-extension matterai.axon-code &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Orbital evals database... "
...
read -p "💻 Do you want to build a new version of the Orbital extension? [currently $current_version] (y/N): " build_extension
