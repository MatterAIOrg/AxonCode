...
package so.matterai.jetbrains.commands
 ...
 */
class AxonCodeAuthProtocolCommand : JBProtocolCommand("so.matterai.jetbrains.auth") {
    private val logger = Logger.getInstance(AxonCodeAuthProtocolCommand::class.java)
    private val coroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    companion object {
        const val COMMAND_ID = "so.matterai.jetbrains.auth"
    ...
    private fun forwardTokenToVSCodeExtension(token: String) {
        coroutineScope.launch {
            try {
                ...
                val vscodeUriString = "vscode://matterai.axon-code/axoncode?token=$token"
