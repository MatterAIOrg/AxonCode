...
package so.matterai.jetbrains.plugin

import so.matterai.jetbrains.core.ExtensionProcessManager
import so.matterai.jetbrains.core.ExtensionSocketServer
import so.matterai.jetbrains.core.ExtensionUnixDomainSocketServer
import so.matterai.jetbrains.core.ISocketServer
import so.matterai.jetbrains.core.ServiceProxyRegistry
import so.matterai.jetbrains.util.ExtensionUtils
import so.matterai.jetbrains.util.PluginConstants
import so.matterai.jetbrains.util.PluginResourceUtil
import so.matterai.jetbrains.webview.WebViewManager
import so.matterai.jetbrains.workspace.WorkspaceFileChangeManager
    ...
    companion object {
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/axoncode/jetbrains/plugin/config/plugin.properties")
