...
package so.matterai.jetbrains.core

import so.matterai.jetbrains.actors.MainThreadBulkEditsShape
import so.matterai.jetbrains.actors.MainThreadClipboardShape
import so.matterai.jetbrains.actors.MainThreadCommandsShape
import so.matterai.jetbrains.actors.MainThreadConfigurationShape
import so.matterai.jetbrains.actors.MainThreadConsoleShape
import so.matterai.jetbrains.actors.MainThreadDebugServiceShape
import so.matterai.jetbrains.actors.MainThreadDiaglogsShape
import so.matterai.jetbrains.actors.MainThreadDocumentContentProvidersShape
import so.matterai.jetbrains.actors.MainThreadDocumentsShape
import so.matterai.jetbrains.actors.MainThreadEditorTabsShape
import so.matterai.jetbrains.actors.MainThreadErrorsShape
import so.matterai.jetbrains.actors.MainThreadExtensionServiceShape
import so.matterai.jetbrains.actors.MainThreadFileSystemEventServiceShape
import so.matterai.jetbrains.actors.MainThreadFileSystemShape
import so.matterai.jetbrains.actors.MainThreadLanguageFeaturesShape
import so.matterai.jetbrains.actors.MainThreadLanguageModelToolsShape
import so.matterai.jetbrains.actors.MainThreadLoggerShape
import so.matterai.jetbrains.actors.MainThreadMessageServiceShape
import so.matterai.jetbrains.actors.MainThreadOutputServiceShape
import so.matterai.jetbrains.actors.MainThreadSearchShape
import so.matterai.jetbrains.actors.MainThreadSecretStateShape
import so.matterai.jetbrains.actors.MainThreadStatusBarShape
import so.matterai.jetbrains.actors.MainThreadStorageShape
import so.matterai.jetbrains.actors.MainThreadTaskShape
import so.matterai.jetbrains.actors.MainThreadTelemetryShape
import so.matterai.jetbrains.actors.MainThreadTerminalServiceShape
import so.matterai.jetbrains.actors.MainThreadTerminalShellIntegrationShape
import so.matterai.jetbrains.actors.MainThreadTextEditorsShape
import so.matterai.jetbrains.actors.MainThreadUrlsShape
import so.matterai.jetbrains.actors.MainThreadWebviewViewsShape
import so.matterai.jetbrains.actors.MainThreadWebviewsShape
import so.matterai.jetbrains.actors.MainThreadWindowShape
import so.matterai.jetbrains.ipc.proxy.createProxyIdentifier
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostCommandsProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostConfigurationProxy
...
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostExtensionServiceProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostFileSystemEventServiceProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostTerminalServiceProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostTerminalShellIntegrationProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWebviewViewsProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWebviewsProxy
import so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWorkspaceProxy
...
//
    ...
    object ExtHostContext {
        ...
        val ExtHostCommands = createProxyIdentifier<ExtHostCommandsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostCommandsProxy")
        val ExtHostConfiguration = createProxyIdentifier<ExtHostConfigurationProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostConfigurationProxy")
        ...
        val ExtHostDocumentsAndEditors = createProxyIdentifier<ExtHostDocumentsAndEditorsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostDocumentsAndEditorsProxy")
        val ExtHostDocuments = createProxyIdentifier<ExtHostDocumentsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostDocumentsProxy")
        ...
        val ExtHostEditors = createProxyIdentifier<ExtHostEditorsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostEditorsProxy")
        ...
        val ExtHostFileSystemEventService = createProxyIdentifier<ExtHostFileSystemEventServiceProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostFileSystemEventServiceProxy")
        ...
        val ExtHostExtensionService = createProxyIdentifier<ExtHostExtensionServiceProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostExtensionServiceProxy")
        val ExtHostLogLevelServiceShape = createProxyIdentifier<Any>("ExtHostLogLevelServiceShape")
        val ExtHostTerminalService = createProxyIdentifier<ExtHostTerminalServiceProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostTerminalServiceProxy")
        val ExtHostTerminalShellIntegration = createProxyIdentifier<ExtHostTerminalShellIntegrationProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostTerminalShellIntegrationProxy")
        ...
        val ExtHostWorkspace = createProxyIdentifier<ExtHostWorkspaceProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWorkspaceProxy")
        val ExtHostWindow = createProxyIdentifier<Any>("ExtHostWindow")
        val ExtHostWebviews = createProxyIdentifier<ExtHostWebviewsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWebviewsProxy")
        ...
        val ExtHostWebviewViews = createProxyIdentifier<ExtHostWebviewViewsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostWebviewViewsProxy")
        val ExtHostEditorInsets = createProxyIdentifier<Any>("ExtHostEditorInsets")
        val ExtHostEditorTabs = createProxyIdentifier<ExtHostEditorTabsProxy>("so.matterai.jetbrains.ipc.proxy.interfaces.ExtHostEditorTabsProxy")
