...
Workflows automate repetitive tasks by defining step-by-step instructions for Orbital to execute. Invoke any workflow by typing `/[workflow-name.md]` in the chat.

<img src="/docs/img/slash-commands/workflows.png" alt="Workflows tab in Orbital" width="600" />

## Creating Workflows

Workflows are markdown files stored in `.orbital/workflows/`:

- **Global workflows**: `~/.orbital/workflows/` (available in all projects)
- **Project workflows**: `[project]/.orbital/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.orbital/workflows` directory:
...
Now you can trigger this workflow by typing `/submit-pr.md` in the chat. Orbital will:
