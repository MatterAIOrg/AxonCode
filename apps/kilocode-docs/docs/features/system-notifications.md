...
Orbital uses system notifications to inform you about:
...
Orbital's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Orbital uses macOS's built-in command to display notifications.
...
**How it works:** Orbital first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
