...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Orbital and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Orbital) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Orbital
...
- Single-client scenarios (one Orbital instance per server)
...
1. The client (Orbital) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Orbital
...
STDIO servers run locally on the same machine as Orbital, which has several important implications:
...
- **Execution**: Starts and stops with Orbital (child process lifecycle)
...
- Start when needed by Orbital
- Not require network configuration
- Need to be installed alongside Orbital or via a package manager
...
## Configuring Transports in Orbital

For detailed information on configuring STDIO and SSE transports in Orbital, including example configurations, see the [Understanding Transport Types](/orbital/features/mcp#understanding-transport-types) section in the Using MCP in Orbital guide.
