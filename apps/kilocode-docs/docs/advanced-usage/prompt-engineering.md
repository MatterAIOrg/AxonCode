...
Prompt engineering is the art of crafting effective instructions for AI models like Orbital. Well-written prompts lead to better results, fewer errors, and a more efficient workflow.

## General Principles

- **Be Clear and Specific:** Clearly state what you want Orbital to do. Avoid ambiguity.
...
It's often helpful to guide Orbital through a "think-then-do" process:

1.  **Analyze:** Ask Orbital to analyze the current code, identify problems, or plan the approach.
2.  **Plan:** Have Orbital outline the steps it will take to complete the task.
3.  **Execute:** Instruct Orbital to implement the plan, one step at a time.
4.  **Review:** Carefully review the results of each step before proceeding.

## Using Custom Instructions

You can provide custom instructions to further tailor Orbital's behavior. There are two types of custom instructions:
...
- Adjust Orbital's tone or personality.

See the [Custom Instructions](/advanced-usage/custom-instructions) section for more details.

## Handling Ambiguity

If your request is ambiguous or lacks sufficient detail, Orbital might:
...
If Orbital doesn't produce the desired results, you can provide feedback by:

- **Rejecting Actions:** Click the "Reject" button when Orbital proposes an action you don't want.
- **Providing Explanations:** When rejecting, explain _why_ you're rejecting the action. This helps Orbital learn from its mistakes.
...
By following these tips, you can write effective prompts that get the most out of Orbital's capabilities.
