...
`list_files` 工具显示指定位置中的文件和目录。它帮助 Orbital 理解您的项目结构并有效地导航代码库。
...
- 当 Orbital 需要理解您的项目结构时
- 当 Orbital 在读取特定文件之前探索可用文件时
- 当 Orbital 映射代码库以更好地理解其组织方式时
- 在使用更针对性的工具（如 `read_file` 或 `search_files`）之前
- 当 Orbital 需要检查项目中特定类型的文件（如配置文件）时
...
- 当开始新任务时，Orbital 可能会列出项目文件以了解其结构，然后再深入研究特定代码。
- 当被要求查找特定类型的文件（如所有 JavaScript 文件）时，Orbital 首先列出目录以知道在哪里查找。
- 当提供代码组织建议时，Orbital 首先检查当前项目结构。
- 当设置新功能时，Orbital 列出相关目录以了解项目约定。
